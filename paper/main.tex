...
As a point of comparison, we consider output from two complementary sets of hydrodynamic simulations as well as satellite imagery derived from MODIS. The primary focus is two observationally constrained large-eddy simulations of deep convection. These high-resolution domain configurations are referred to as ``giga-LES'' simulations because their numerical grids contain over a billion points. The second comparison is to a multimodel ensemble of coarser cloud-resolving simulations from the Radiative-Convective Equilibrium Model Intercomparison Project \citep[RCEMIP;][]{wing2018, wing2020}. 
...
As a final analysis of STEAM outputs, we turn to renderings of simulated clouds. \citet{palmer2016} proposed a ``climatic Turing test'' as a standard for model fidelity: if simulated output cannot be distinguished from observations of the real atmosphere, then for all practical purposes there is no difference between them. Visual realism is of course a bounded form of evidence, but it can be meaningful when paired with quantitative statistics, allowing one to build intuition about what STEAM does well and what it does not. As we will show, renderings suggest concrete future research directions, and for that alone the exercise is quite valuable.

We use the cloud visualization engine \verb|cloudyview| (CITATION). Because \verb|cloudyview|'s physically-based Monte Carlo engine is computationally intractable for thick clouds, we use a ray-marching algorithm that uses tuned heuristics for pixel lighting but physically accurate viewing geometry. The ray-marching algorithm is computationally inexpensive enough that a consumer GPU can render views in real time, as if in a video game. An interactive demo of STEAM-simulated clouds, alongside hydrodynamic comparisons, is available for this purpose at (URL).

Visualizations are produced from two sets of STEAM simulations. Both make use of ``nested'' domains, which are produced by first simulating a ``parent'' domain, and then continuing the cascade over a small sub-region of that parent, as described in Appendix \ref{sect:steam algorithm details}. Using the square MODIS-matched subdomains of Sect. (ADD GEO REF), we first simulate a $32\times 32\,\unit{km}$ nested domain, spanning the full depth of the parent domain, to a final horizontal grid spacing of $62.5\,\unit{m}$, corresponding to a grid size for the nest of $512\times512$. A secondary nested domain spanning $8\times8\,\unit{km}$ horizontally and $1$ to $5\,\unit{km}$ vertically is then simulated to a horizontal grid spacing of approximately $7.8\,\unit{m}$, corresponding to a final horizontal grid size of $1024\times1024$. Each nested domain is centered exactly within its parent simulation.
% TODO (other computer): add vertical grid SIZES (level counts) for the two nests -- supplement Table (tab:steam runs) gives horizontal grids and vertical extents only; read nz from output files.
