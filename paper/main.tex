...
\begin{align}
A_h &= C_{h, \ell}\, g_h \frac{W_{h,\ell}}{\langle W_{h,\ell}\rangle_\mathbf{x}(z)} \frac{S_\ell}{\langle |S_\ell|\rangle}, \\
A_q &= C_{q, \ell}\, g_q \frac{W_{q,\ell}}{\langle W_{q,\ell}\rangle_\mathbf{x}(z)} \frac{S_\ell}{\langle |S_\ell|\rangle},
  \label{apxeq:amplitude propto gradient normalized}
\end{align}
where $W_\ell$ is the local advective weight, $\langle W_\ell \rangle_\mathbf{x}(z)$ its realized horizontal mean at each level, $g_\Phi$ a bound factor, $S_\ell$ the flux weighting factor, and $C_\ell$ the mean absolute turbulon amplitude. We will take each factor in turn. [CLAUDE: This equation shows each factor normalized separately, but the code normalizes the \emph{product} $g_\Phi W_{\Phi,\ell} S_\ell$ per level by its mean absolute value over the turbulon centers, then multiplies $C_\ell$ (2026-07-27/28 rulings, audit item 35). The distinction is load-bearing: $W$ and $S_\ell$ are correlated --- the flux is large where the running field's gradients are large, and the correlation compounds down-cascade, so separate normalization does not normalize the product ($\langle \hat W |S_\ell| \rangle$ grew $\sim$9$\times$ over 7 octaves and cancelled the $\ell^{H_h}$ ladder). The joint norm also makes the implemented $\langle |S_\ell| \rangle$ convention (the noise alone, $\langle |e^\gamma - 1| \rangle$ over centers, not $|S_\ell|$ with the flux in it) inert for the delivered amplitude. The supplement states the joint form; this equation, the $\langle W_\ell \rangle$ sentence below (its ``neglecting the bounding term'' caveat dissolves --- the taper is inside the norm), and the $\langle |S_\ell| \rangle$ sentence need the same rewrite.]

The first factor $C_\ell$ is a normalization factor representing the mean absolute turbulon amplitude at each scale, and the unit normalization for the weight and flux factors $W_{\Phi,\ell}$ and $S_\ell$ ensures that $C_\ell$ alone sets the overall variability of scalar perturbations, at least for regions away from the bounds set by $g_\Phi$. The factors $C_\ell$ are constrained by the Lovejoy-Schertzer law of scalar advection (Eqn. \ref{eq:LS scalar directional structure functions}) such that
\begin{align}
...
The next factor in Eqn. \ref{apxeq:amplitude propto gradient normalized}, $S_\ell$, is the amplitude of the flux component of a turbulon, equal to
\begin{align}
S_\ell = \left(e^{\gamma} - 1\right)\mathcal{F}_{>\ell}, \label{apxeq:scalar flux factor}
\end{align}
whose convolution generates the flux perturbation in Eqn. \ref{eq:flux at scale}. The normalization $\langle |S_\ell| \rangle$ in Eqn. \ref{apxeq:amplitude propto gradient normalized} is the global mean absolute value of $S_\ell$ over the turbulon centers. This implements the hypothesis discussed in Sect. \ref{sect:distribution of turbulon amplitudes in space} that the amplitude of a turbulon's scalar component is proportional to the amplitude of its flux component, both being set by the strength of the eddy's overturning motion. The factor is shared between $h$ and $q_t$ because a single eddy transports both scalars. Because $S_\ell$ is signed and centered at zero, scalar perturbations mix in both directions and inherit the skewness of the flux perturbations. Note that $S_\ell$ depends only on size classes larger than $\ell$, so each cascade step is fully determined by the classes preceding it.
...
\end{align}
where $\ell_z/\ell$ is the turbulon aspect ratio of Eqn. \ref{apxeq:vertical turbulon size class}. The normalization by the realized horizontal mean $\langle W_\ell \rangle_\mathbf{x}(z)$ enforces the mean absolute turbulon amplitude of Eqn. \ref{apxeq:mean turbulon amplitude} exactly at every level and size class, neglecting the bounding term $g_\Phi$. Strictly speaking, a more natural normalization convention would be to use the ensemble mean $\langle W_{\Phi,\ell}\rangle_i(z)$. For a sufficiently large domain containing many outer scales, $\langle W\rangle_\mathbf{x}(z)\approx \langle W_{\Phi,\ell}\rangle_i(z)$. Smaller domains may incur some error, particularly for larger size classes containing fewer turbulons. Note that, as described in Sect. \ref{sect:distribution of turbulon amplitudes in space}, levels for which the ensemble mean profile is constant have vanishing $W_{\Phi,\ell}$, and the scalar component of the turbulon amplitudes vanishes with it.
