...
The anisotropy implied by Eqn \ref{eq:LS directional structure functions} -- where the Hurst exponent is a function of direction -- is also straightforward to model, simply by allowing the turbulon aspect ratio to vary as a function of the turbulon size (Fig. \ref{fig:cascade slice}). To be consistent with Eqn. \ref{eq:LS directional structure functions}, a simulation must set the mean absolute turbulon amplitude such that 
\begin{align}
...
\caption{Vertical slice along an ($x$--$z$) plane of an anisotropic cascade. Ellipses represent the support of individual turbulons of varying size $k$ and location. Turbulon aspect ratios follow Eqn. \ref{eq:turbulon aspect ratio scaling} such that large turbulons are horizontally elongated whereas turbulons near in size to the spheroscale $l_s$ have unit aspect ratio. Number density per size class is proportional to $k^{-(1+H_z)}$, ensuring turbulons of every size class cover the domain.} \label{fig:cascade slice}
\end{figure*}

For simulations, both the shape and relative amplitude at each scale of the turbulons is set by the horizontal hurst exponent $H_h$, the anisotropy exponent $H_v$, and the spheroscale. The canonical values for Lovejoy-Schertzer turbulence are $H_h=1/3$ and $H_z=5/9$ (Eqn. ()), while the spheroscale must be determined empirically. Different values of $H_h$ and $H_z$ can also be used to simulate different turbulent flows, for example isotropic turbulence where $H_h=1/3, H_z=1$. [CLAUDE: The simulations in this paper run $H_h = 0.45$ (\texttt{steam.constants.hurst\_horizontal}; the supplement states it, and both $\lambda$ and the bound-buffer closure depend on it). The main text currently gives only the canonical $1/3$ --- a sentence stating and motivating the production value (e.g. the observed $H_h \approx 0.4$ of your dropsonde preprint) is still needed. Audit item C19's remaining half.]
...
\section{Analysis of STEAM simulation output and comparison to hydrodynamic simulations}

As a point of comparison, we consider output from two complementary sets of hydrodynamic simulations. The primary focus is two observationally constrained large-eddy simulations of deep convection. These high-resolution domain configurations are referred to as ``giga-LES'' simulations because their numerical grids contain over a billion points. The second comparison is to a multimodel ensemble of coarser cloud-resolving simulations from the Radiative-Convective Equilibrium Model Intercomparison Project \citep[RCEMIP;][]{wing2018, wing2020}. 
...
From the RCEMIP archive we use the nine models with explicit convection that provide usable three-dimensional snapshot output for the 300 K channel: SAM, CM1, SCALE, UCLA-CRM, three configurations of the Met Office Unified Model, and two configurations of ICON. Each model provides three snapshots separated by ten days near the end of the integration, taken after the simulations have equilibrated. Two archived models are excluded for data integrity reasons (see supplement). A complete specification of the comparison data and analysis is provided in the supplement.
Notably, SAM appears in both comparison sets, at $100\,\unit{m}$ resolution for the giga-LES cases and at $3\,\unit{km}$ resolution in RCEMIP, so the two evaluations bracket the model across a factor of 30 in grid spacing using a shared dynamical core.

In any hydrodynamic simulation, statistics computed at the grid scale are often strongly affected by numerical approximations such as hyperdiffusion or filtering that tend to damp grid-scale variability. It is commonly remarked that, due to such approximations, a model's ``effective'' resolution is a few times larger than the grid spacing (citation). As we will show, our findings using fluctuation functions, which measure variability across different scales, also suggest that hydrodynamic models produce too little grid-scale variability. STEAM, however, produces almost exactly as much grid-scale variability as would be expected by scale invariance, and therefore STEAM's effective resolution is equal to its grid spacing.

A distinct but closely related challenge is that simple statistics such as standard deviation depend strongly on the resolution used to compute them, especially for scale invariant fields \citep{lovejoy2013}. To account for both of these methodological subtleties, for all one-point statistics such as standard deviations, we first coarsen hydrodynamic outputs in $2\times 2\times 2$ blocks to reduce numerical artifacts, and then configure STEAM comparisons such that the horizontal and vertical resolutions are matched between STEAM and the hydrodynamic comparison. Analysis functions that measure variability across scales, such as fluctuation functions and cloud fractal metrics, are computed at native resolution in order to show numerical artifacts directly.

\subsection{Initialization of STEAM simulations}

Because STEAM requires parameters such as mean profiles to be specified as inputs, we construct a matching STEAM configuration for each individual hydrodynamic case. We refer to a given configuration of the hyrdodynamic model as the ``host'' model. Most importantly, the mean vertical profiles of the prognostic variables $q_t$ and $h$ are required as inputs to STEAM, and these are computed from the host model output. STEAM also requires a profile for the spheroscale, a constant $c$ specifying the amount of intermittency in the flux, and a constant for the outer scale. The outer scale $L$, which represents the size of the largest turbulon, is set to half the length of the domain for square configurations and one quarter the length of the longer domain for channel configurations, a choice made to represent the constrained geometry of the channels. For the spheroscale profile, we use a uniform value of $10\,\unit{m}$ which is consistent with observations \citep{dewitt2025preprint}. It is less clear what the value for the flux perturbation scaling factor $c$ should be. For simplicity, we consider two cases: $c\in{0.05, 0.17}$. As we discuss in Section (), in the real atmosphere the spheroscale is likely to display large spatial variability, so the constant used here is a key approximation. 
...
The second dimension is the ``ensemble'' fractal dimension, whic quantifies both how convoluted individual cloud perimeters are in addition to the relative frequency of small and large clouds,  The dimension is obtained via a linear regression to a logarithmically-transformed plot of the count of cloud-edge pixels that are separated less than a distance $r$. The computation of both fractal dimensions follows the recommendations of \citet{dewitt2026}.
...
As an additional point of comparison, we compute the fractal metrics for a selection of 72 satellite images, collected using MODIS between January 1-10. The selection was filtered to remove granules containing sun glint and are entirely over ocean, so that a simple reflectivity-based threshold may be used to define cloud boundaries. More details are provided for the MODIS dataset in \citet{dewitt2026}.
...
For both STEAM and the giga-LES volumes, simulated albedo fields are constructed by first computing a three-dimensional optical depth field from the liquid and ice water contents using the bulk-optics relationships of SAM, as implemented in the \verb|cloudyview| package and matching the treatment of \citet{dewitt2024}:
\begin{align}
  \tau = (\text{PLACEHOLDER}). \label{eq:bulk optics}
\end{align}
% CLAUDE: placeholder -- the exact bulk-optics relationships live in cloudyview (glimpse.py / optics code) on the other machine; fill in the liquid and ice extinction formulas (LWP/IWP with effective radii) from there.

The optical depth is then summed vertically within each column to obtain a column optical depth $\tau_c$, from which visual albedo is computed using the two-stream approximation for a conservatively scattering cloud layer under an overhead sun \citep[e.g.,][]{petty2006}:
\begin{align}
  A = \frac{(1-g)\,\tau_c}{(1-g)\,\tau_c + 2}, \label{eq:two stream albedo}
\end{align}
with asymmetry parameter $g = 0.85$. With the solar zenith adjustment described in the supplement, MODIS reflectivity and model albedo are directly comparable for clouds. For both, cloud masks are defined by thresholding the albedo or reflectivity for thresholds $R \in \{0.1, 0.2, 0.3\}$, so that the masks are directly comparable to satellite cloud masks defined by thresholding reflectance \citep{dewitt2026}. Because $A$ increases monotonically with $\tau_c$, each albedo threshold is equivalent to a threshold in column optical depth, with the three values of $R$ corresponding to $\tau_c \approx 1.5$, $3.3$, and $5.7$.
...
\subsection{Renderings of STEAM clouds}

Our last evaluation of STEAM simulations is more qualitative. 

An advantage of STEAM over traditional hydrodynamic models is that simulations of nested domains at much finer resolution are straightforward to simulate by simply continuing the cascade over a small subregion (described in more detail in Appendix \ref{sect:steam algorithm details}). First, a $64\times 64\,\unit{km}$ nested domain, spanning the full depth of the parent domain, is simulated to a final grid size of $1024\times1024$, corresponding to a horizontal grid spacing of $62.5\,\unit{m}$. Then, a secondary nested domain spanning only the lowest $4\,\unit{km}$ vertically and $32\,\unit{km}$ horizontally is simulated with a final horizontal grid size of $2048\times2048$ and a horizontal grid spacing of approximately $16\,\unit{m}$. Each nested domain is centered exactly within its parent simulation.
...
\begin{align}
  \mathfrak{T}_k(||r||) = \left(\delta -\frac{\pi^2||r||^2}{k^2}\right)\exp \left(-\frac{\pi^2||r||^2}{2k^2}\right) \label{apxeq:turbulon shape}
\end{align}
...
\end{align}
The familiar one-dimensional Mexican hat would have $\delta = 1$, but $\delta$ must be modified in order to keep $\mathfrak{T}_k(||r||)$ zero mean, both because the wavelet is three-dimensional and because it is resolved on a coarse grid. We use $\delta=2.9678$, a constant computed numerically for default ``sparsity factors'' defined below.

The turbulon aspect ratio $k_h/k_z$ is defined by
\begin{align}
  \begin{cases}
    k_z>l_s; \qquad k_z = l_s \left(\frac{k}{l_s}\right)^{H_z}, \\
    k_z \le l_s; \qquad k_z = k,
  \end{cases}
   \label{apxeq:vertical turbulon size class}
\end{align}
which recovers Eqn. \ref{eq:turbulon aspect ratio scaling} at scales larger than $l_s$ and is isotropic at smaller scales. This piecewise aspect ratio scaling function was found to produce more visually realistic sub-spheroscale clouds (not shown). However, all simulations that are analyzed quantitatively here are fully within the anisotropic regime due to the small spheroscales considered. The visualization in Fig. () is the only simulation containing sub-spheroscale turbulons.
...
\end{align}
where $W_k$ is the local advective weight, $\langle W_k \rangle_\mathbf{x}(z)$ its realized horizontal mean at each level, $g_\Phi$ a bound factor, $S_k$ the flux weighting factor, and $C_k$ the mean absolute turbulon amplitude. We will take each factor in turn. [CLAUDE: This equation shows each factor normalized separately, but the code normalizes the \emph{product} $g_\Phi W_{\Phi,k} S_k$ per level by its mean absolute value over the turbulon centers, then multiplies $C_k$ (2026-07-27/28 rulings, audit item 35). The distinction is load-bearing: $W$ and $S_k$ are correlated --- the flux is large where the running field's gradients are large, and the correlation compounds down-cascade, so separate normalization does not normalize the product ($\langle \hat W |S_k| \rangle$ grew $\sim$9$\times$ over 7 octaves and cancelled the $k^{H_h}$ ladder). The joint norm also makes the implemented $\langle |S_k| \rangle$ convention (the noise alone, $\langle |e^\gamma - 1| \rangle$ over centers, not $|S_k|$ with the flux in it) inert for the delivered amplitude. The supplement states the joint form; this equation, the $\langle W_k \rangle$ sentence below (its ``neglecting the bounding term'' caveat dissolves --- the taper is inside the norm), and the $\langle |S_k| \rangle$ sentence need the same rewrite.]
...
\end{align}
with $n_b = 3$, so that the amplitude tapers exactly where the remaining cascade is likely to carry the field into a bound and not before. Viewed as a stochastic process, fluctuations decelerate in proportion to the remaining distance in the same way that the fluctuations of a geometric random walk decelerate near zero, and crossings become rare rather than routine. [CLAUDE: Eqn. \ref{apxeq:bound buffer} above is implemented with the sum closed geometrically over the WHOLE remaining ladder, $b_{\Phi,k} = n_b\, C_{\Phi,k}/(1 - 2^{-H_h/n_c})$ --- at $H_h = 0.45$, $n_c = 1$ that is $3.73\, n_b C_k$, vs $2.95\, n_b C_0$ for the truncated five-class sum of a channel run, 27\% wider at the outer class. Truncating at the run's own finest class would make the taper depend on where the run stops, breaking the nest--root identity. The supplement states the closed form; the equation here needs it too.] Importantly, $g_\Phi$ is not normalized and has values $\le 1$. Global normalization of $g_\Phi$ would imply that scalar values near a bound in one part of the domain would (nonphysically) increase variance in other parts of the domain that are not close to the bound. [CLAUDE: This argument was overturned by measurement (2026-07-28 ruling): with the taper OUTSIDE the norm, the effective ladder became $C_k \langle g_k \rangle$ with $\langle g \rangle$ strongly scale-dependent (buffers shrink down-cascade), which flattened the production $h$ Haar ladder and inverted $q_t$'s. The code now includes $g_\Phi$ inside the per-level product norm: proximity to a bound \emph{redistributes} the level's amplitude toward far-from-bound cells rather than suppressing the level total. These two sentences need to argue the redistribution reading, or at least stop asserting the opposite of what the model does.]
